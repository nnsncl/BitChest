...
export default function Currency() {
  ...
  useEffect(() => {
    coins &&
      axios
        .get(
          `https://api.coingecko.com/api/v3/coins/markets?vs_currency=eur&ids=${id}&order=market_cap_desc&per_page=10&page=1&sparkline=false`
        )
        .then((response) => {
        ...
        <section className="md:w-2/3 w-full">
          <div className=" mb-6 pb-6 border-b-2 border-gray-800">
            <div className="flex items-start gap-3 mb-3">
              <h2 className="text-6xl font-bold">
                {currency.current_price &&
                  currency.current_price.toLocaleString(undefined, {
            ...
            <ul className="flex items-baseline gap-3 text-xs">
              <li className="flex items-center text-xs text-gray-700 mb-2 mr-3">
                {currency.name}&nbsp;Price&nbsp;
                <span className="uppercase">({currency.symbol})</span>
              </li>
              <li className="text-gray-700 ">
                High:&nbsp;
                <span className="text-white">{currency.high_24h}€</span>
              </li>
              <li className="text-gray-700 ">
                Low:&nbsp;
                <span className="text-white">{currency.low_24h}€</span>
              </li>
          ...
          <ul className="text-xs md:flex-row flex-col flex items-start justify-between gap-6">
            <li className="flex flex-col gap-2">
              ...
              <p>
                {currency.market_cap &&
                  currency.market_cap.toLocaleString(undefined, {
                    maximumFractionDigits: 2,
                  })}
                €
              </p>
            ...
            {currency.fully_diluted_valuation && (
              <li className="flex flex-col gap-2">
                ...
                <p>
                  {currency.fully_diluted_valuation.toLocaleString(undefined, {
                    maximumFractionDigits: 2,
                  })}
                  €
                </p>
            ...
            <li className="flex flex-col gap-2">
              ...
              <p>
                {currency.total_volume &&
                  currency.total_volume.toLocaleString(undefined, {
                    maximumFractionDigits: 2,
                  })}
                €
              </p>
            ...
            <li className="flex flex-col gap-2">
              ...
              <p>
                {currency.circulating_supply &&
                  currency.circulating_supply.toLocaleString(undefined, {
                    maximumFractionDigits: 2,
                  })}
              </p>
            </li>
      ...
      <section className="flex md:flex-row flex-col items-start w-full justify-between gap-6">
        ...
        <article className="md:w-1/3 w-full bg-gray-800 p-6 rounded-xl">
          <h3 className="text-xl font-bold mb-9">
            <span className="uppercase">{currency.symbol}</span>&nbsp;Price
            Statistics
          </h3>
          <ul className="text-gray-700">
            <p className="font-light text-xs mb-3">
              {currency.name}&nbsp;Price Today
            </p>
              ...
              <p className="flex flex-col items-end">
                <span className="text-white">
                  {currency.price_change_24h &&
                    currency.price_change_24h.toLocaleString(undefined, {
              ...
              <p className="flex flex-col items-end">
                <span className="text-white">
                  {currency.market_cap_change_24h &&
                    currency.market_cap_change_24h.toLocaleString(undefined, {
                      maximumFractionDigits: 3,
                    })}
                  €
                </span>
                ...
                >
                  {currency.market_cap_change_percentage_24h &&
                    currency.market_cap_change_percentage_24h.toLocaleString(
                      undefined,
                      { maximumFractionDigits: 3 }
                    )}
                  %
